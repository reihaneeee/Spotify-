      ...
      {item.artist && (
        <p 
          onClick={(e) => {
            ...
            const found = allUsers.find(u => u.userType === 'artist' && (u.id === item.artistId || u.artistName === item.artist || u.username === item.artist));
            const artistId = found ? found.id : (item.artistId || item.artist);
            // ۲. انتقال مستقیم به مسیر آرتیست
