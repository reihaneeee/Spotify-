...
import { notifyFollowersAboutNewRelease } from '../../utils/notificationEngine'; 

const UploadWork = ({ onAdd, onUpdate, onCancel, editData, albums = [] }) => {
  ...
  // استیت‌های ترک‌های آلبوم
  const [albumTracks, setAlbumTracks] = useState([
    { id: Date.now(), title: '', audioFileName: '', audioUrl: '', audioData: '', audioFile: null, lyrics: '' }
  ]);
  ...
  useEffect(() => {
    if (editData) {
      const workType = editData.type || (editData.tracks && editData.tracks.length > 0 ? 'album' : 'single');
      setFormData({
        title: editData.title || '',
        type: workType,
        albumId: editData.album || editData.albumId || '',
        genre: editData.genre || '',
        releaseDate: editData.releaseDate || editData.release_date || '',
        collaborators: editData.collaborators || editData.featured_artist_names || '',
        lyrics: editData.lyrics || '',
      });
      
      const coverSrc = editData.cover || editData.cover_image || '';
      setCoverPreview(coverSrc);
      setCoverUrl(typeof coverSrc === 'string' && coverSrc.startsWith('http') ? coverSrc : '');
      
      if (workType === 'album' && Array.isArray(editData.tracks) && editData.tracks.length > 0) {
        setAlbumTracks(editData.tracks);
      } else {
        const audioSrc = editData.audioData || editData.audioUrl || editData.audio_file || '';
        setAudioPreview(audioSrc);
        setAudioFileName(editData.audioFileName || (typeof audioSrc === 'string' ? audioSrc.split('/').pop() : ''));
        setAudioUrl(typeof audioSrc === 'string' && audioSrc.startsWith('http') ? audioSrc : '');
      }
    }
  }, [editData]);
  ...
  const handleTrackChange = (id, field, value) => {
    setAlbumTracks(prev => prev.map(track => track.id === id ? { ...track, [field]: value } : track));
  };
  ...
  const handleTrackFileSelect = (id, file) => {
    ...
    const reader = new FileReader();
    reader.onloadend = () => {
      setAlbumTracks(prev => prev.map(track => 
        track.id === id 
          ? { ...track, audioData: reader.result, audioFileName: file.name, audioFile: file, audioUrl: '' } 
          : track
      ));
    };
    reader.readAsDataURL(file);
  ...
  const addTrack = () => setAlbumTracks([...albumTracks, { id: Date.now(), title: '', audioFileName: '', audioUrl: '', audioData: '', audioFile: null, lyrics: '' }]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.type === 'single' && !audioFile && !audioUrl && !audioPreview && !isEditing) {
      setError('Audio file or URL is required for singles.');
      return;
    }

    if (formData.type === 'album') {
      const hasValidTrack = albumTracks.some(t => t.title.trim() && (t.audioFile || t.audioUrl || t.audioData));
      if (!hasValidTrack && !isEditing) {
        setError('Please add at least one track with title and audio file to the album.');
        return;
      }
    }
    ...
    const interval = setInterval(async () => {
      ...
      if (progress >= 100) {
        ...
        if (formData.type === 'single') {
          ...
          workData.audioUrl = audioUrl || audioPreview || '';
        ...
        if (isEditing) {
          await onUpdate(workData);
        } else {
          await onAdd(workData);
          try {
        ...
        if (!isEditing) {
          setFormData({ title: '', type: 'single', albumId: '', genre: '', releaseDate: '', collaborators: '', lyrics: '' });
          clearCover();
          clearAudio();
          setAlbumTracks([{ id: Date.now(), title: '', audioFileName: '', audioUrl: '', audioData: '', audioFile: null, lyrics: '' }]);
        }
      }
  ...
  return (
    <form className={styles.uploadForm} onSubmit={handleSubmit}>
      ...
      {formData.type === 'single' && albums && albums.length > 0 && (
        <div className={styles.formGroup}>
          <label>Assign to Album (Optional)</label>
      ...
      {formData.type === 'single' ? (
        ...
        <div className={styles.formGroup}>
          ...
          {albumTracks.map((track, index) => (
            <div key={track.id || index} style={{ background: '#1a1a1a', padding: '1rem', borderRadius: '8px', border: '1px solid #2a2a2a', marginBottom: '1rem' }}>
