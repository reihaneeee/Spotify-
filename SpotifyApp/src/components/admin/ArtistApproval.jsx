...
import styles from '../../styles/ArtistApproval.module.css';
...
const ArtistApproval = ({ artists, onApprove, onReject }) => {
  ...
  if (selectedArtist) {
    // اصلاح نام فیلد به snake_case مطابق با جنگو
    const portfolioLinks = selectedArtist.portfolio_links || selectedArtist.portfolioLinks || (selectedArtist.portfolio ? [selectedArtist.portfolio] : []);
    const requestDate = selectedArtist.date_joined ? new Date(selectedArtist.date_joined).toLocaleDateString() : new Date().toLocaleDateString();
    ...
    return (
      <div className={styles.artistDetail} style={{ animation: 'fadeIn 0.3s ease' }}>
        ...
        <h3 style={{ marginTop: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          {/* تصحیح کلیدهای نام آرتیست */}
          Review Artist: {selectedArtist.artist_name || selectedArtist.display_name || selectedArtist.email}
        </h3>
        ...
        <div style={{ background: '#1e1e1e', padding: '1.5rem', borderRadius: '12px', marginTop: '1.5rem', border: '1px solid #2a2a2a' }}>
          ...
          <div style={{ marginTop: '1.5rem', paddingTop: '1.5rem', borderTop: '1px solid #2a2a2a' }}>
            ...
            {portfolioLinks.length > 0 ? (
              ...
              <span style={{ color: '#ef4444', paddingLeft: '1.5rem' }}>No portfolio provided</span>
            )}
          </div>
        </div>
        ...
        <div className={styles.tableWrapper}>
          <table className={styles.table}>
            ...
            <tbody>
              {artists.map((artist) => (
                <tr key={artist.id}>
                  {/* اصلاح کلیدهای نمایش نام در جدول */}
                  <td>{artist.artist_name || artist.display_name || 'N/A'}</td>
                  <td>{artist.email}</td>
                  <td>
